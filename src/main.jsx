    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <section className="intro-row">
          ...
          <button className="refresh-button">↻ <span>Verileri yenile</span></button>
        </section>
        ...
        <div className="content-grid">
          <section className="panel races-panel">
            <div className="panel-heading"><div><p className="eyebrow">PROGRAM</p><h2>İstanbul koşuları</h2></div><span className="live-badge"><i /> CANLI PROGRAM</span></div>
          ...
          <section className="panel analysis-panel">
            ...
            <div className="probability"><div className="prob-head"><span>Olasılık dağılımı</span><small>Son 10 koşu + pist + jokey</small></div><div className="bar"><span style={{width: `${race.confidence}%`}} /></div><div className="prob-labels"><span><i className="dot green" /> {race.favorite} <b>{race.confidence}%</b></span><span><i className="dot orange" /> {race.favorites[1]} <b>{Math.max(9, race.confidence - 57)}%</b></span><span><i className="dot gray" /> Diğerleri <b>{Math.max(13, 100 - race.confidence - Math.max(9, race.confidence - 57))}%</b></span></div></div>
        ...
        <section className="coupon-section"><div className="section-title"><div><p className="eyebrow">AKILLI KUPONLAR</p><h2>Bugün için hazır kombinasyonlar</h2></div><p>Modelin risk ve bütçe dengesine göre oluşturduğu seçenekler</p></div><div className="coupon-grid">{coupons.map((item, index) => <button key={item.name} onClick={() => setSelectedCoupon(index)} className={selectedCoupon === index ? 'coupon-card selected' : 'coupon-card'}><div className="coupon-top"><span className="coupon-tag">{item.tag}</span><span className="coupon-arrow">↗</span></div><h3>{item.name}</h3><p>{item.game} <span>·</span> {item.legs}</p><div className="coupon-bottom"><div><strong>{item.chance}%</strong><small>modelin tutma<br />olasılığı</small></div><span className="cost">{item.cost}</span></div></button>)}</div><div className="disclaimer"><span>ⓘ</span><p>Bu oranlar geçmiş performans ve model tahminidir; kesin sonuç veya kazanç garantisi değildir. Sorumlu oynayın.</p><button>Model metodolojisi →</button></div></section>
        <footer><span>HorseRide Intelligence v0.1</span><span>Veriler demo amaçlıdır · TJK entegrasyonu sonraki aşamada</span></footer>
      </main>
    </div>
  )
}
